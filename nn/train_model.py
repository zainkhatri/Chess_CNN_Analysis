...
class EnhancedChessCNN(nn.Module):
    """Chess position evaluation and move prediction CNN with enhanced architecture"""
    def __init__(self, input_channels=12, num_filters=128, num_residual_blocks=10, num_output_moves=1968):
        ...
        # Residual blocks with bottleneck architecture (like ResNet)
        ...
        # Policy head (move prediction) with dropout for regularization
...
def focal_loss(pred, target, gamma=2.0, alpha=0.25):
    """
    Focal Loss for addressing class imbalance in the move prediction task
    Formula: -alpha * (1 - p)^gamma * log(p) where p is the predicted probability of the target class
    """
...
def train_model(model, train_loader, val_loader, optimizer, policy_criterion, value_criterion, 
    ...
    """Train the chess CNN model with enhanced training strategy"""
    ...
    for epoch in range(num_epochs):
        ...
        for positions, values, moves in train_pbar:
            ...
            # Calculate loss with focal loss for policy to address class imbalance
            if epoch < 5:
                # Use regular cross entropy for initial stability
                policy_loss = policy_criterion(policy_output, moves)
            else:
                # Switch to focal loss after initial stability
                policy_loss = focal_loss(policy_output, moves, gamma=2.0)
            
            value_loss = value_criterion(value_output, values)
            
            # Combined loss with adaptive weighting
            # Give even more weight to policy as training progresses
            policy_weight = min(0.9, 0.7 + epoch * 0.01)  # Gradually increase to 0.9
        ...
        with torch.no_grad():
            for positions, values, moves in val_pbar:
                ...
                # Calculate loss with same weighting as training
                policy_loss = policy_criterion(policy_output, moves)
                value_loss = value_criterion(value_output, values)
        ...
        if scheduler:
            # Use policy accuracy for scheduling
            scheduler.step(1 - val_policy_acc)  # Invert since we want to maximize accuracy
        ...
        # Track both validation loss and policy accuracy 
        improved = False
        ...
        # Also track best policy accuracy and save separately
        ...
        if improved:
            patience_counter = 0
            # Also save as general best model
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        if patience_counter >= early_stopping_patience and epoch >= 20:
            # Only trigger early stopping after at least 20 epochs
    ...
    # Plot learning rate history if available
...
def main():
    """Main training function with best possible settings"""
    ...
    try:
        data = torch.load(data_path, map_location=torch.device('cpu'))  # Always load to CPU first
        ...
        # Analyze the class distribution for move prediction
    ...
    train_size = int(0.85 * len(dataset))  # Increase training set size slightly
    val_size = len(dataset) - train_size
    train_dataset, val_dataset = random_split(dataset, [train_size, val_size])
    
    # Create dataloaders with smarter settings
    ...
    # Define optimizer with more sophisticated settings
    ...
    # Custom learning rate scheduler with history tracking
